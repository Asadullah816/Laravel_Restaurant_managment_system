            ...
            <div class="p-4 rounded bg-secondary h-100">
                ...
                <form action="{{ route('chefUpdate',$chef->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3 form-floating">
                        <input type="text" value="{{$chef->name}}" class="form-control" name="name" id="floatingInput"
                            placeholder="name@example.com" required>
                        <label for="floatingInput">Name</label>
                    </div>
                    <div class="mb-3 form-floating">
                        <input type="text" value="{{$chef->speciality}}"  name="speciality" class="form-control" id="floatingPassword"
                            placeholder="Price" required>
                        <label for="floatingPassword">Speciality</label>
                    </div>
                    <div class="mb-3">
                        <label for="formFileSm" class="form-label">Chef Image </label>
                        <input type="file" value="{{$chef->image}}" name="image" class="form-control form-control-sm bg-dark"
                            id="formFileSm" >
                            <img class="food-image-admin" src="{{ asset('storage/' . $chef  ->image) }}" alt="">
                    </div>
